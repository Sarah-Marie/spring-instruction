...
public class MovieController {
	...
	@GetMapping("/by-movie-rating")
	public JsonResponse listByMovieRating(@RequestParam String rating) {
		JsonResponse jr = null;
		List<Movie> movies = movieRepo.findAllByRating(rating);
		if (rating.equals ("PG")) {
			jr = JsonResponse.getErrorInstance("No movie rating found for movie:  "+rating+".");
		}
		else  {
			jr = JsonResponse.getInstance(movies);
		}
